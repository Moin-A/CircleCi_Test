describe('Visual regression for elements in webqoof section', () => {
    ...
    cy.visit("/collections/webqoof")
    cy.get('#container').matchImageSnapshot('collectionLanding',{coverage: "fullpage"})
    })
    it('compare the screenshot of the webqoof section with the one taken in beta env', () => {
    Cypress.currentTest.retries(1)    
    cy.visit(Cypress.env("thequint-beta-hindi")+'/collections/webqoof')
    cy.get('#container').matchImageSnapshot('collectionLanding',{coverage: "fullpage"})
    })
    })